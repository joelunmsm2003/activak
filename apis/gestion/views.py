...
def reporte(request):
    ...
    if request.method == 'GET':
        ...
        for j in range(len(data)):
            ...
            base = OrigBase.objects.filter(cliente=data[j]['cliente']).values('id','telefono','contacto__nombre','estado__nombre','accion__nombre','observacion','nombre_agente','tadicional','id_orig_base_c__campana').order_by('-fgestion')

            fmt = '%Y-%b-%d %H:%M:%S'

            for x in range(len(base)):

                base[x]['fgestion'] = OrigBase.objects.get(id_orig_base=base[x]['id']).fgestion.strftime(fmt)
        ...
        writer.writerow(['Cuenta','DNI','Agente','Contacto','Accion','Estado','Observacion','Fecha Gestion','Telefono','Campana'])

        print 'Csv...'

        for d in data:

            writer.writerow([d['contador'],d['cliente'],d['registros']['nombre_agente'],d['registros']['contacto__nombre'],d['registros']['accion__nombre'],d['registros']['estado__nombre'],d['registros']['observacion'],d['registros']['fgestion'],d['registros']['telefono'],d['registros']['id_orig_base_c__campana']])
